...
modeler <- function(df, fm_term = "y ~ x", model_type = "lm", predNo = 1, filename = "auto") {
  ...
  # filter the relevant variabes from the model selection. predNo = 1: 1 relevant predictor; predNo = 2: 2 relevant predictors
  ...
  # save the data
  if (summary(bm)$coefficients[pred1,4] < 0.05) {
